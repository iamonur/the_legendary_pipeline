...
if __name__ == "__main__":
    ...
    a = bfs_python(lines[:-1], frm=(int(top)-1, int(left)))
    w = bfs_python(lines[:-1], frm=(int(top), int(left)-1))
    d = bfs_python(lines[:-1], frm=(int(top)+1, int(left)))
    s = bfs_python(lines[:-1], frm=(int(top), int(left)+1))
