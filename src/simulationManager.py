...
class Sim_Feb:
    def __init__(
        ...
        map_generator=cellularAutomata.elementary_cellular_automata, 
        map_polisher=caPolisher.CAPolisher_MinArea, 
        sprite_planner=spritePlanner.dualSpritePlanner, 
        spin=spinner.SpinClass_Game4, 
        parser=spinParser.spinParser, 
        searcher=spinner.A_Star_Game4, 
        player=player.MazeGameClass, 
        feed=randomFeeder_Generic):
        ...
        while(True):
            ...
            if modelling_success:
                ...
                if modelling_success:
                    ...
                    """
                    If modelling is not successful, don't wait for astar to run.
                    """
                    astar_time = time.time()
                    astar_moves = self.astar_ag(map_).perform()
                    astar_time = time.time() - astar_time
                    astar_score, astar_terminal = self.player(action_list=astar_moves, level_desc=map_).play()



                    astar_moves = list(map(str,astar_moves))
                    avatar = list(map(str, avatar))
                    spin_line  = "Spin score: {}\nSpin terminal:{}\nSpin time: {}\nSpin moves: {}".format(
                        spin_score, (spin_terminal and (spin_score > 0)), modelling_time, "*".join(avatar))
                    astar_line  = "ASTAR score: {}\nASTAR terminal: {}\nASTAR time:{}\nASTAR moves: {}".format(
                        astar_score,(astar_terminal and (astar_score > 0)),astar_time, "*".join(astar_moves))

                    return level_line, spin_line, astar_line, (astar_terminal and (astar_score > 0))
            return level_line, None, None, None

sizes_to_try = [8,16,24]
games_implemented = [
...
class Sim_Feb_Wrapper:
    ...
    def record_episode_(self, num, size, game_name, levelStr):
        filename = "XX" + str(self.magic) + "_" + str(size) + "_" + game_name + "_" + str(num) 
    ...
    def run_simulation(self):
        ...
        for game in self.my_types:
            for size in self.my_sizes:
                sim = Sim_Feb(level_size=size,sprite_planner=game[0],spin=game[1],player=game[2])
                for i in range(0,self.numOfEps):
                    print("---")
                    level_str, spin_str, mcts_str, _ = sim.pipeline()
                    if spin_str == None:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        sim_no += 1
    """
    Sim_Feb_Wrapper(50).run_simulation()
