...
class MCTS_Wrapper:
    def __init__(self, map):
        ...
        self.root = my_maze_state.initFromMap(map)
        self.rnode= my_maze_node(self.root)
        self.search = MCTS(self.rnode)

    def perform(self):
        self.search.best_action(1)
...
class my_maze_move(abstract_game_action):
    def __init__(self, line, char, mover, button): #Mover is either 'avatar' or 'enemy'
        self.line = line
        self.char = char
        self.mover = mover
        self.button = button
...
class my_maze_state(two_player_abstract_game_state):
    ...
    def __init__(self, walls, avatar, enemy, portal, lines, chars): #Done-sies
        ...
        if self.portal == self.avatar: #Avatar stepped on portal.
            self.game_result = 1
            os.system("echo 'WON' >> asd.txt")
            retryasd = False
            print("b")
        elif self.avatar == self.enemy: #Enemy stepped on avatar.
            self.game_result = -1
            os.system("echo 'LOST' >> asd.txt")
            retryasd = True
            print("a")
    ...
    def __append_legal_actions(self): #Done-sies
        ...
        retlist.append(my_maze_move(self.avatar[0], self.avatar[1], 'Avatar', 'Skip'))
    
        #Only walls break you
        if self.avatar[0] > 0: #Cannot go up otherwise
            temp = (self.avatar[0]-1, self.avatar[1])
            if temp not in self.walls:
                retlist.append(my_maze_move(self.avatar[0]-1, self.avatar[1], 'Avatar', 'W'))
    
        if self.avatar[0] < self.lines-1: #Cannot go down otherwise
            temp = (self.avatar[0]+1, self.avatar[1])
            if temp not in self.walls:
                retlist.append(my_maze_move(self.avatar[0]+1, self.avatar[1], 'Avatar', 'S'))
        
        if self.avatar[1] < self.chars-1: #Cannot go right otherwise
            temp = (self.avatar[0], self.avatar[1]+1)
            if temp not in self.walls:
                retlist.append(my_maze_move(self.avatar[0], self.avatar[1]+1, 'Avatar', 'D'))
        
        if self.avatar[1] > 0: #Cannot go left otherwise
            temp = (self.avatar[0], self.avatar[1]-1)
            if temp not in self.walls:
                retlist.append(my_maze_move(self.avatar[0], self.avatar[1]-1, 'Avatar', 'A'))
    ...
    def move(self, action): #Done-sies
        ...
        if action.mover == 'Avatar':
            ...
            os.system("echo '{}' >> asd.txt".format(action.button))
    ...
    def _move_dummy_enemy(self): #LEFT, UP, RIGHT, DOWN
        ...
        if (line, char) in self.walls:
            return self.move(my_maze_move(self.enemy[0], self.enemy[1], 'Enemy', 'Dont care'))
        return self.move(my_maze_move(line, char, 'Enemy', 'Dont care'))
