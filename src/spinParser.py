...
class spinParser_Soko_2:

    def __init__(self, outFile="tempfile.txt", execFile="../spin/temp.out", trailFile_1="temp.pml1.trail", trailFile_2="temp.pml2.trail", mp=None):
        if mp is None:
            raise cannotWinException("You dont know how to use this.")
        ...
        self.output_file = outFile
        self.executable_file = execFile
        self.trail_file_1 = trailFile_1
        self.trail_file_2 = trailFile_2

    def __get_trail_output(self, trailer):
        os.system("{} -r -S {} > {}".format(self.executable_file, trailer, self.output_file))
    ...
    def perform(self):
        self.__get_trail_output(self.trail_file_1)
        moves = self.__parse_moves(self.__parse_trail_output())
        if moves is None:
            raise cannotWinException("Sokoban game cannot be won!")

        self.__get_trail_output(self.trail_file_2)
        moves_2 = self.__parse_moves(self.__parse_trail_output())
        if moves_2 is None:
            raise cannotWinException("Sokoban game cannot be won!")

        if len(moves) == len(moves_2):
            raise cannotWinException("Moves are eq.")
