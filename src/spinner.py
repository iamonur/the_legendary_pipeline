...
class SpinClass_Sokoban_LTL_Outside():
  def __init__(self, map, goals):
    self.map = map
    self.length = len(map)
    self.width = len(map[0])
...
typedef row{{
  ...
  row map[{}];
"""
promela_header_for_game_4_new_ltl = """
typedef row{{
  byte a[{}]
  }}
  bit win = 0;
  int straight_moves = 0;
  int turning_moves = 0;
  row map[{}];
"""
...
typedef row{{ //This is for creating 2-D arrays, since they are not supported.
  ...
  int next_y;
"""
promela_header_for_game_3_smart_ltl_movement = """
typedef row{{ //This is for creating 2-D arrays, since they are not supported.
    byte a[{0}];
  }}
  c_code{{\#include "../spin/bfs.c"}};
...
"""
promela_avatar_game_3_ltl_movement = """
proctype avatar_same_goal(int x; int y){
  map[x].a[y] = 2;
  byte w, a, s, d;
  bit foo;
...
ltl  { [] !win };
"""
promela_ltl_formula_movement = """
// LTL Formula : In any time, win never be true.
// Counter-Example will be generated -> A scenario to win.
ltl  { []  (!win || (turns<10))};
"""
promela_ltl_formula_2 = """
// LTL Formula : In any time, win never be true.
// Counter-Example will be generated -> A scenario to win.
ltl  { [] !win };
"""
...
class SpinClass_Game4_New_LTL():
  ...
  def create_spin(self):
    ...
    formatted_header = promela_header_for_game_4_new_ltl.format(self.length+2, self.width+2)
    self.promela_whole_file = self.promela_whole_file.format(
			promela_comment_01,
			promela_comment_02,
			formatted_header,
...
class SpinClass_Sokoban_100k_optimal_init():
  def __init__(self, map, goals):
    self.map = map
    self.length = len(map)
    self.width = len(map[0])
  ...
  def fix_map(self):
    ...
    for lineNum, line in enumerate(temp_map):
      #TODO: Remember, you assumed a square there.
      self.list_walls.append((lineNum, 0))
      self.list_walls.append((0,lineNum))
      self.list_walls.append((self.length, lineNum))
      self.list_walls.append((lineNum, self.width))
      for chNum, ch in enumerate(line):
        if ch == '0':
          temp_map[lineNum][chNum] = '.'
        elif ch == '1':
  ...
  def create_spin(self):
    ...
    formatted_init = promela_init_sokoban_modified.format(
      avatar_y = self.avatar_location[0] + 1,
      avatar_x = self.avatar_location[1] + 1,
      length = self.length + 1,
      length2 = self.length,
...
class SpinClass_Game3_smart_LTL_movement():
  def __init__(self, map):
    self.map = map
    self.length = len(map)
    self.width = len(map[0])#Assuming a rectangle
  ...
  def create_spin(self):
    ...
    formatted_header = promela_header_for_game_3_smart_ltl_movement.format(self.length+2, self.width+2)
    self.promela_whole_file = self.promela_whole_file.format(
			promela_comment_01,
			promela_comment_02,
			formatted_header,
...
if __name__ == "__main__":
  ...
  sp.perform()
  ss = SpinClass_Game3_smart_LTL_movement(sp.getMap())
